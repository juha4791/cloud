...
class CloudPredictionTrainer:
    ...
    def __init__(self, data_folder="./gk2a_data", model_save_dir="./models"):
    ...
    def create_data_loaders(self, train_data, val_data, batch_size=8, sequence_length=4):
        ...
        # 더미 데이터셋 (실제 구현에서는 processed_data 활용)
...
def run_full_training_pipeline():
    ...
    try:
        ...
        # 실제 데이터가 있는지 확인
        if not os.path.exists('./gk2a_data') or len(os.listdir('./gk2a_data')) < 10:
            print("⚠️  충분한 GK2A 데이터가 없습니다.")
            print("먼저 gk2a_downloader.py를 실행하여 데이터를 수집하세요.")
            return
        
        # train_data, val_data, file_list = trainer.prepare_training_data(
        #     start_date, end_date, train_ratio=0.8
        # )
        
        # 2. 데이터 로더 생성
        # train_loader, val_loader = trainer.create_data_loaders(
        #     train_data, val_data, batch_size=4
        # )
        
        # 3. 모델 훈련 (더미 데이터로 테스트)
        print("⚠️  실제 데이터 대신 더미 데이터로 훈련 데모를 실행합니다.")
        
        # 더미 데이터셋 생성
        dummy_files = [f"dummy_file_{i}.nc" for i in range(20)]
        train_dataset = CloudDataset(dummy_files[:16], sequence_length=4)
        val_dataset = CloudDataset(dummy_files[16:], sequence_length=4)
        
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=2, shuffle=True)
        val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=2, shuffle=False)
        
        model, train_losses, val_losses = trainer.train_and_evaluate(
            train_loader, val_loader, num_epochs=5  # 데모용 짧은 훈련
        )
...
# 훈련 스크립트 저장
with open('train_cloud_model.py', 'w', encoding='utf-8') as f:
    f.write(training_script)

print("✅ 모델 훈련 스크립트 생성 완료: train_cloud_model.py")